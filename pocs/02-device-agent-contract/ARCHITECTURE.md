...
- 규칙은 `USV/AUV/ROV`의 중심 행동을 먼저 정하는 방식이 아니라, 센서/상황 조건에 따라 추천을 바꾸는 방식입니다.
- 기본 우선순위는 `안전 > 임무 > 효율`입니다.
- 현재 핵심 규칙은 `속도 초과`, `위험한 수심`, `낮은 조도`, `battery 경고/위기`, `home/base 이탈`, `미확인 타깃 알림`입니다.
