...
| 자율 임무  | 임무 생성 및 스케줄링             | 부분 구현 | 사용자 UI 기반 임무 생성은 없습니다. System Agent가 Alert 기반으로 Response와 mission steps를 구성합니다.                   |
| 자율 임무  | 자율 임무 실행 및 추적            | 부분 구현 | `task.assign` dispatch, `mission.result` 수신, Response 상태 갱신 흐름이 구현되어 있습니다.                                 |
| 자율 임무  | 임무 대체 및 경로 재계획          | 부분 구현 | System Agent가 reservation, queue, revalidation, 일부 reassign 흐름을 지원합니다. 전역 최적화는 없습니다.                   |
| 상황 인지  | 상황 인지형 관제                  | 구현됨    | `event.report` 수신 -> Event 저장 -> severity 판단 -> Alert 생성 -> Response 계획/배정 -> `mission.result` 반영 흐름입니다. |
...
| ------------------ | --------------------------------------------------------------- | --------------------------------------------- |
| 작전 지휘관        | Alert/Response 상태 확인, 대응 흐름 검토                        | 자연어 임무 생성, 계획 승인, 임무 스케줄링 UI |
| 운영 관제사        | 3D 대시보드, 디바이스 상태, 경보 관리 화면 사용                 | 실시간 조치 옵션 승인 및 수동 재배정          |
| 현장 유지보수 인력 | 배터리, 연결 상태, healthcheck 기반 상태 확인                   | 예측 정비, 장비별 정비 이력 관리              |
| AI 엔지니어/개발자 | Agent state, Event/Alert/Response ledger, A2A inbox/outbox 확인 | Agent trace, LLM 평가, 모델별 비교 도구       |
...
| 경보 관리     | `client/alerts.html`        | Registry `/alerts`, `/responses`, `/alerts/{id}/ack` 기반 경보/대응 확인 |
| 디바이스 상세 | `client/device-detail.html` | 특정 디바이스의 센서 stream, 위치, 배터리, 연결 상태 확인                |
| Registry UI   | `server/registration/ui/*`  | Registry API 기반 디바이스, alert, response, settings 화면               |

## 4. 현재 지원 시나리오

### 기뢰 탐지 이벤트 기반 대응

현재 구현에 가장 가까운 사용 흐름은 자연어 임무 생성이 아니라, 이벤트 기반 대응입니다.
...
| 5    | System Agent                       | 현재 연결된 capable device 중 대상 장비를 선택합니다. middle parent가 있으면 middle을 경유합니다. |
| 6    | System Agent                       | HTTP A2A `/message:send`로 `task.assign`을 dispatch합니다.                                        |
| 7    | Middle/Lower Agent                 | task를 처리하고 `mission.result`를 상위로 보고합니다.                                             |
| 8    | System Agent                       | Response의 `dispatch_result`, 실행 결과, 최종 상태를 갱신합니다.                                  |
...
| ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| Registry Server 중단                 | Registry API가 응답하지 않으면 디바이스 등록, Event/Alert/Response 원장 기록, 상태 조회가 실패합니다. 일부 Device Agent는 기존 identity cache로 Moth 초기화를 시도할 수 있습니다. | Registry가 canonical owner이므로 원장 기록과 assignment 계산은 Registry 복구 전까지 정상 보장되지 않습니다.                         |
...
| A2A dispatch 실패                    | System Agent가 dispatch 결과를 Response의 `dispatch_result`에 기록하고, 실패 시 Response를 `failed` 또는 후속 처리 상태로 갱신합니다.                                             | 네트워크 재시도 정책과 운영자 승인 UI는 제한적입니다. 실패 원인 분석은 `dispatch_result`와 agent state 확인이 필요합니다.           |
| capable device 없음                  | System Agent가 target을 찾지 못하면 Response를 `failed`로 기록합니다.                                                                                                             | 임무 재계획 또는 사용자 수동 선택 UI는 아직 없습니다.                                                                               |
| capable device가 모두 reservation 중 | capability는 있으나 즉시 사용 가능한 장비가 없으면 Response를 `queued`로 둘 수 있고, 이후 재검토합니다.                                                                           | queue는 최소 구현입니다. mission 간 preemption이나 전역 스케줄 최적화는 지원하지 않습니다.                                          |
| 중복 mission result                  | System Agent는 `response_id + step_id + task_id + source_agent_id` 기준으로 중복 결과를 무시합니다.                                                                               | 중복 제거 기준 밖의 비정상 payload 정합성 검증은 추가 보완이 필요합니다.                                                            |
