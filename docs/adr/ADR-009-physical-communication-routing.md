...
Agent {
  ...
  // 동적 (실시간 변경)
  environment_state: "surface" | "submerged"  // 현재 위치
...
Event 발행:
  {
    ...
    data: { from: "surface", to: "submerged" }
  }
    ↓
...
Before (수면):
  ...
  AgentConnection[RF] = Active
  AgentConnection[Acoustic] = Standby

Event: ENV_STATE_CHANGED (surface → submerged)
  ↓

After (수중):
  AUV.active_mediums = ["acoustic"]  // ← 자동 업데이트
  AgentConnection[RF] = Inactive (RF Module Sleep)
  AgentConnection[Acoustic] = Active (Acoustic Modem Wake)
```
...
Rule 1: "수중 진입 시 대역폭 제한"
  IF (Event: ENV_STATE_CHANGED to="submerged")
  THEN (
...
class DeviceAgent {
  private lastEnvironmentState = "unknown"
  
  async monitorEnvironment() {
    const depth = await sensor.getWaterDepth()
    const newState = depth > 2 ? "submerged" : "surface"
    ...
    if (newState !== this.lastEnvironmentState) {
      ...
      // Event 발행
      await moth.publish("env-state-changed", {
        agent_id: this.id,
...
function updateActiveMediums(agent: Agent): string[] {
  ...
  // Step 2: 환경에 따라 필터링
  if (environment_state === "submerged") {
...
3️⃣ System Agent 응답:
  a. Agent.environment_state = "submerged" 갱신
...
5️⃣ 수면 복귀:
  AUV 센서: depth = 0.5m (수면 도달)
    ↓
  ENV_STATE_CHANGED: "submerged" → "surface"
    ↓
  System Agent:
