...
## **P11: 이벤트 단일 처리 (Event Single Processing)**

**원칙**: 하나의 이벤트 흐름은 한 번에 하나의 Agent만 처리가 가능해야 한다.

**정의**:

- 모든 Event는 단일 `target_agent`를 명시해야 함 (다중 `target_agents` X)
- 동시에 여러 Agent가 같은 Event를 처리하는 상황을 배제
- Event Routing 계층이 올바른 Agent를 판단하고 단독으로 전달

**왜 필요한가?**

- **상태 일관성**: 여러 Agent가 동일 Event 처리 → 상태 불일치 위험
- **책임 명확성**: Event 처리 결과 추적 가능 (누가 처리했는지 명확)
- **예측 가능성**: Event 흐름이 명확하고 순차적

**효과**:

- Event Log에서 누가 처리했는지 명확히 추적 가능
- Event 처리 오류 시 책임 주체 즉시 파악
- Event 상태 불일치 버그 원천 차단
