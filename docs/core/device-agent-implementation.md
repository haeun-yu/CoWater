# Device Agent 구현 가이드 (Implementation Guide)
...
## 2. Device Agent 초기화 & 등록 (Initialization & Registration)
...
## 3. Task 수행 판단 (Capability-Driven Task Execution)
...
## 4. 물리 통신 드라이버 (Physical Communication Driver)
...
## 6. Local Safety Behavior (로컬 안전 행동)
...
## 7. 환경 상태 감지 (Environment State Management)
...
## 8. A2A Protocol 상세 (Agent-to-Agent Communication)
...
- [a2a-protocol.md](./a2a-protocol.md) - A2A Protocol 규격
- [communication-driver.md](./communication-driver.md) - 물리 통신 추상화
- [ADR-009: Physical Communication Routing](../adr/ADR-009-physical-communication-routing.md)
- [schema.md](./schema.md) - Task, Agent 데이터 구조
