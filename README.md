...
# 에이전트 활성화 / 비활성화
PATCH http://localhost:8001/agents/{agent_id}/enable
PATCH http://localhost:8001/agents/{agent_id}/disable

# 자율성 레벨 변경 (L1 / L2 / L3)
PATCH http://localhost:8001/agents/{agent_id}/level
Body: {"level": "L2"}
```
...
|------|--------|------|
| `REDIS_URL` | `redis://localhost:6379` | Redis 주소 |
| `CORE_API_URL` | `http://localhost:8000` | Core Backend URL |
...
| 변수 | 기본값 | 설명 |
|------|--------|------|
| `NEXT_PUBLIC_API_URL` | `http://localhost:8000` | Core REST API |
| `NEXT_PUBLIC_WS_URL` | `ws://localhost:8000` | WebSocket 허브 |
| `NEXT_PUBLIC_AGENTS_URL` | `http://localhost:8001` | Agent Runtime |
