...
| 용어              | 의미                                                                                                         |
| ----------------- | ------------------------------------------------------------------------------------------------------------ |
| `Registry Server` | `server/registration`. 디바이스 등록, 상태 저장, healthcheck 반영, assignment 계산, alert/response 원장 제공 |
...
| `Healthcheck`     | 생존·위치·배터리 등의 최소 상태 스냅샷                                                                       |
...
| 컴포넌트        | 책임                                                                                                |
| --------------- | --------------------------------------------------------------------------------------------------- |
| Registry Server | 등록·조회 API, healthcheck 반영, 위치/연결 상태 갱신, parent assignment 계산, alert/response ledger |
| System Agent    | alert 해석, 타깃 선정, 최고 수준 의사결정, A2A task dispatch, response 기록                         |
| Middle Agent    | 하위 디바이스 조율, A2A 라우팅/중계, 하위 상태 인지                                                 |
| Lower Agent     | 임무 수행, telemetry/healthcheck 발행, 결과/이벤트 상향 보고                                        |
...
예외 조치 후에는 반드시 상위에 A2A 또는 healthcheck로 상황을 알린다.
...
| 생존/위치/배터리 상태    | Moth `device.healthcheck`        | Registry Server가 구독               |
...
- 모든 에이전트는 `1초` 주기로 healthcheck를 발행한다.
- `3초` 이상 healthcheck가 없으면 Registry가 offline으로 처리한다.
- 위치 데이터를 생성하거나 읽을 수 있는 에이전트는 `latitude`, `longitude`를 healthcheck에 포함해야 한다.
- 배터리 데이터를 생성하거나 읽을 수 있는 에이전트는 배터리 값을 healthcheck에 포함해야 한다.
- 현재 구현에서 lower / middle agent는 Moth `device.healthcheck`를 사용하고, System Agent는 Registry keepalive로 `last_seen_at`을 갱신한다.

### Telemetry 규칙

- telemetry는 센서 데이터 스트림이다.
- Registry의 canonical 상태 갱신 기준은 telemetry가 아니라 healthcheck다.
- telemetry만 발행하고 healthcheck에 위치를 넣지 않으면 Registry 위치 정보는 stale해질 수 있다.
...
- Registry 상태와 로컬 체감 상태가 다르면, 다음 healthcheck 전까지 Registry를 공용 기준으로 본다.
...
| 장애                | 기본 대응                                          |
| ------------------- | -------------------------------------------------- |
| Healthcheck timeout | offline 처리, 필요 시 자식 재할당                  |
...
- healthcheck, telemetry, A2A 로그가 실제로 남아야 한다.
