...
class BaseAgentRuntime(ABC):
    ...
    async def _registry_keepalive_loop(self) -> None:
        ...
        interval = max(1, int(self.config.get("registry", {}).get("healthcheck_interval_seconds", 1)))
        logger = logging.getLogger(__name__)
        while True:
    ...
    async def _handle_moth_event(self, event: dict[str, Any]) -> None:
        """수신한 Moth MEB 이벤트를 역할별 handle_moth_message 추상 메서드로 라우팅"""
    ...
    async def execute_role_request(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        return await self._execute_role(parameters)

    @abstractmethod
    async def _execute_role(self, parameters: dict[str, Any]) -> dict[str, Any]:
        """각 서브클래스가 역할별 실행 로직을 구현한다."""
        ...
    ...
    # ─── Shared helpers (used by multiple role subclasses) ───────────────────
    ...
    def _reserve_device(self, *, device_id: int, response_id: str, step_id: str, task_id: str, reason: str) -> None:
        self._device_allocations[device_id] = {
            "device_id": device_id,
    ...
    # ─── Shared mission execution methods ────────────────────────────────────
    ...
    # ─── shared handle_command_with_llm, generate_mission_proposal, etc ──────
    # These are cross-role methods kept in base for reuse
    ...
    # ─── Mission execution (shared across DeviceBridge / MissionPlanner) ────
    ...
    async def generate_mission_proposal(
        ...
        *,
        ...
        if not steps:
            steps = self._build_generic_steps_for_goal(goal or mission_type, devices, location)
            ...
            bundle = await self.generate_mission_proposal(
                {
                    **payload,
                    ...
                    "_approach": strategy.get("approach"),
                },
                allow_suppression=False,
                _preset_mission_type=mission_type,
