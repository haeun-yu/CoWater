...
_MAX_CACHED_ALERTS = 12
_MAX_FOCUSED_PLATFORMS = 3
_MAX_PLATFORM_SUMMARY = 5
_MAX_ALERT_SUMMARY = 5
_MAX_HISTORY_TURNS = 3
_MAX_HISTORY_CHARS = 240
_MAX_ALERT_MESSAGE_CHARS = 80
...
- 모든 답변은 한국어로
- 한국어 띄어쓰기와 문장부호를 자연스럽게 유지
- 문장을 붙여 쓰지 말고, 1~3문장마다 줄바꿈
- 항목형 답변은 번호 목록 또는 불릿 목록으로 정리
- "보좌관:" 같은 접두사는 붙이지 말고 바로 답변 본문부터 시작

[시스템 제어 명령어 안내]
사용자가 시스템을 직접 제어하려면 아래 명령어 형식을 사용하도록 안내하십시오.
명령어는 자동으로 감지되어 확인 후 실행됩니다.

에이전트 제어 (권한: admin):
- "[에이전트] 켜줘" / "[에이전트] 꺼줘" — 활성화/비활성화
- "[에이전트] 레벨 L1|L2|L3" — 자율성 레벨 설정 (L1=알림, L2=반자동, L3=완전자율)
- "[에이전트] 실행해줘" — 수동 실행 (권한: operator)

지원 에이전트: cpa/충돌위험, zone/구역감시, anomaly/이상행동, distress/조난, report/보고서

경보 처리 (권한: operator):
- "경보 [UUID] 인지" / "경보 [UUID] 해결"

명령어는 operator-dev 또는 admin-dev 토큰이 필요하며 audit_logs에 기록됩니다."""
...
class ChatAgent(Agent):
    ...
    name = "AI 보좌관"
    description = (
        "현재 해양 상황을 컨텍스트로 활용하여 운항자와 실시간 소통하는 AI 챗봇"
    )
    agent_type = "ai"
    ...
    async def on_alert(self, alert: dict) -> None:
        ...
        if alert.get("generated_by") == self.agent_id:
            return
        self._recent_alerts = [alert, *self._recent_alerts[: _MAX_CACHED_ALERTS - 1]]
    ...
    def _build_situation_context(self, focus_platform_ids: list[str] | None) -> str:
        ...
        lines = [
            f"[현재 시각] {now}",
            f"[관제 중인 선박] 총 {len(self._recent_reports)}척",
        ]
        ...
        if focus_platform_ids:
            ...
            if focused:
                lines.append("[주목 선박]")
                for r in focused[:_MAX_FOCUSED_PLATFORMS]:
        ...
        if self._recent_reports:
            lines.append("[선박 목록 (최대 5척)]")
            for r in list(self._recent_reports.values())[:_MAX_PLATFORM_SUMMARY]:
        ...
        for a in active[:_MAX_ALERT_SUMMARY]:
            sev = a.get("severity", "?").upper()
            atype = a.get("alert_type", "?")
            pids = ", ".join(a.get("platform_ids", []))
            msg = str(a.get("message", ""))[:_MAX_ALERT_MESSAGE_CHARS]
