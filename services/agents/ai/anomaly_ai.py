...
class AnomalyAIAgent(Agent):
    ...
    async def on_alert(self, alert: dict) -> None:
        ...
        if alert.get("generated_by") == self.agent_id:
            return
        # Rule 기반 선별 결과만 AI 분석 대상으로 허용
        if alert.get("generated_by") != "anomaly-rule":
            return
        ...
        if settings.ai_min_severity == "critical" and alert.get("severity") != "critical":
            return
        ...
        pid = platform_ids[0]
        cooldown_key = f"{alert.get('alert_type')}:{pid}"
        now_ts = time.time()
